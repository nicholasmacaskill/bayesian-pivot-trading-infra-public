    ...
    try:
        ...
        for arg in cmdline:
            ...
            if arg.startswith(patterns.DEBUG_PORT_FLAG):
                ...
                is_dev_mode = os.path.exists(_config("developer_mode.lock"))
    ...
    try:
        ...
        for conn in connections:
            if conn.status == 'ESTABLISHED' and conn.raddr:
                ...
                # Check for connections to blacklisted domains
                # We prioritize speed, so reverse resolution is only done if not in private/trusted IP space
                if mode != 'learn':
                    try:
                        # Reverse DNS to check for malicious hostnames
                        hostname, _, _ = socket.gethostbyaddr(remote_ip)
                        for blacklisted in getattr(patterns, 'BLACKLISTED_DOMAINS', []):
                            if blacklisted.lower() in hostname.lower():
                                return {
                                    "detected": True,
                                    "critical": True,
                                    "title": "🚨 MALICIOUS DOMAIN CONNECTION",
                                    "summary": f"Process '{name}' (PID: {pid}) is connected to blacklisted domain: {hostname} ({remote_ip})",
                                    "remote_ip": remote_ip,
                                    "hostname": hostname
                                }
                    except (socket.herror, socket.gaierror):
                        # Could not resolve, might still be malicious but we lack domain confirmation
                        pass
