    ...
    try:
        ...
        if not is_browser:
            if any(b in name.lower() for b in ['chrome', 'brave', 'edge', 'arc']):
                 logging.debug(f"DEBUG_SCAN: Process '{name}' (PID: {pid}) blocked by tight prefix filter")
            return None
            
        # Optimization: Skip memory scanning for noisy sandboxed helpers
        if any(h in name for h in ['Helper', 'Renderer', 'GPU', 'Utility']):
            return None
