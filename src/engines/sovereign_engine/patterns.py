...
def _config(f): return os.path.join(SMC_ROOT, 'data', f)
def _run(f):    return os.path.join(SMC_ROOT, 'run_data', f)
...
def load_dynamic_whitelist():
    ...
    try:
        whitelist_path = _config("whitelist.json")
...
MALICIOUS_LINKS = [
    ...
    r'https?://[^/]+\.pkg$',
    r'https?://[^/]+\.zip$',
    r'https?://(?:www\.)?istreameast\.app'
]

# Domains that should be blocked at the network/process connection level.
BLACKLISTED_DOMAINS = [
    'istreameast.app'
]
...
MEMORY_SCAN_INTERVAL = int(os.getenv('MEMORY_SCAN_INTERVAL', '180'))           # seconds
INTEGRITY_CHECK_INTERVAL = int(os.getenv('INTEGRITY_CHECK_INTERVAL', '600'))   # seconds
LAUNCH_SERVICES_CHECK_INTERVAL = int(os.getenv('LAUNCH_SERVICES_CHECK_INTERVAL', '300'))  # seconds
KEYCHAIN_MONITOR_INTERVAL = int(os.getenv('KEYCHAIN_MONITOR_INTERVAL', '60'))  # seconds
